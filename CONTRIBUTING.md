...
# Change directory to the jupyterlite_html_exporter directory
...
```bash
pip uninstall jupyterlite_html_exporter
```

In development mode, you will also need to remove the symlink created by `jupyter-builder develop`
command. To find its location, you can run `jupyter labextension list` to figure out where the `labextensions`
folder is located. Then you can remove the symlink named `jupyterlite-html-exporter` within that folder.
